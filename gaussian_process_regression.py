...
def plot_prediction_color(filename, material, method):
    ...
    for k in range(1,50):  #improvement of the dataset we remove the worst tbr values and we had a better enrichment configuration to replace it
        ...
        if method=='random':
            ...
            new_enrichment_value.append(row_max_tbr['enrichment_value'][0] + add_enrichment[0])
            new_enrichment_value.append(row_max_tbr['enrichment_value'][1] + add_enrichment[1])
            new_enrichment_value.append(row_max_tbr['enrichment_value'][2] + add_enrichment[2])
            print('new enrichment fraction', new_enrichment_value)
...
if __name__ == '__main__':
    ...
    plot(fig,show_link=True,filename = 'plots_new_neutron_source/prediction_results_3_layers_halton.html',image='png')